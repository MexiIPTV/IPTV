...
//Modificaciones hechas por Miguel Angelo 03092024
// Function to create a single directory for the article within the /wiki folder
function createPageDirectory($id) {
	//Utilizada en Save.php para el guardado de articulos
	global $conf;

	// Define the path to the /wiki directory
	$wikiDir = $conf['datadir']. '/wiki';

	// Construct the path for the new article directory
	$articleDir = $wikiDir . '/' . $id;

	// Check if the directory already exists
	if (!is_dir($articleDir)) {
		// Attempt to create the directory
		if (mkdir($articleDir, 0755, true)) {
			echo "Directory created: $articleDir\n";
		} else {
			echo "Failed to create directory: $articleDir\n";
		}
	} else {
		echo "Directory already exists: $articleDir\n";
	}
}